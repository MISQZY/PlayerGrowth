 ...
 * FlectoneGrowth state - height, gender, and growth progress - without going
 * through PlaceholderAPI as a string-based intermediary or reaching into
 * FlectoneGrowth's internal classes.
 *
 * <p>Obtain an instance through Bukkit's {@link org.bukkit.plugin.ServicesManager},
 * FlectoneGrowth's standard integration point for other plugins:
 * <pre>{@code
 * RegisteredServiceProvider<FlectoneGrowthAPI> provider =
 *         Bukkit.getServicesManager().getRegistration(FlectoneGrowthAPI.class);
 * if (provider != null) {
 *     FlectoneGrowthAPI api = provider.getProvider();
 * }
 * }</pre>
 * The service is only registered while FlectoneGrowth is enabled - always
 * guard against a {@code null} registration rather than hard-depending on
 * it being present.</p>
 *
 * <p>Every per-player method only has data for players FlectoneGrowth is
 ...
 * zero or false.</p>
 */
public interface FlectoneGrowthAPI {
